...
It can be shown that all the tasks cannot be completed
in fewer than 4 rounds, so the answer is 4.
...
class Solution:
    def minimumRounds(self, tasks: List[int]) -> int:
        ...
        frequencies = self.get_frequencies(tasks)
        rounds = 0
        keys = frequencies.keys()
        for key in keys:
            if frequencies[key] == 1:
                return -1
            rounds += frequencies[key] // 3
            reminder = frequencies[key] % 3
            if reminder > 0:
                rounds += 1

        return rounds

    def get_frequencies(self, tasks):
