...
represents the XY-plane. The balloons are represented as a 2D integer array
points where points[i] = [xstart, xend] denotes a balloon whose horizontal
diameter stretches between xstart and xend. You do not know the
...
the number of arrows that can be shot.
...
class Solution:
    def findMinArrowShots(self, points: List[List[int]]) -> int:
        ...
        for start, end in points:
            ...
            current_start = max(start, prev_start)
            current_end = min(end, prev_end)
